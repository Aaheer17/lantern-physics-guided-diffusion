...
class MVNHead(torch.nn.Module):
    ...
    Provides two penalties:
      ...
      - per-sample MVN term:     (1/2B) * sum_i (y_i - mu)^T Sigma^{-1} (y_i - mu)
    ...
    def __init__(self, stats_path_or_dict, device, mode="mean", lambda_weight=1e-3):
        ...
        if isinstance(stats_path_or_dict, str):
            ...
            # allow passing preloaded arrays
    ...
    def _mahalanobis_no_grad(self, diff: torch.Tensor) -> torch.Tensor:
        # diff: [..., 45]
        y = torch.cholesky_solve(diff.unsqueeze(-1), self.stats.L)  # solve L L^T y = diff
        return (diff.unsqueeze(-2) @ y).squeeze(-1).squeeze(-1)     # scalar per item

    def _mahalanobis(self, diff: torch.Tensor) -> torch.Tensor:
        # differentiable form (don’t detach model outputs!)
        y = torch.cholesky_solve(diff.unsqueeze(-1), self.stats.L)  # shape [..., 45, 1]
        return (diff.unsqueeze(-2) @ y).squeeze(-1).squeeze(-1)

    def forward(self, vecs_45: torch.Tensor) -> torch.Tensor:
        """
        vecs_45: [B, 45]  (your per-sample vector, e.g., layer-sum energies)
        Returns: scalar MVN/MCLT penalty to add to your base loss
        """
        mu = self.stats.mu              # (45,)
        B  = vecs_45.shape[0]

        if self.mode == "mean":
            mean_b = vecs_45.mean(dim=0)              # (45,)
            diff   = mean_b - mu                      # (45,)
            ...
            loss   = 0.5 * md2.mean()                 # (1/2B) sum => same up to constant
        else:
