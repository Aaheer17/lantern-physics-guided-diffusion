...
def plot_losses(train_losses, val_losses, save_path="loss_plot.png",documenter=None):
    ...
    plt.savefig(documenter.get_file(save_path))
    plt.close()
    print(f"Loss plot saved to {save_path}")
def plot_ui_dists(gen_us, ref_us, xlim=(-0.05, 1.05), ratio_ylim=(0.5, 1.5), num_bins=64, color="#0000cc", quantile_bins=False, skip_quantiles=0, documenter=None,LBL=None):
    print("Inside the plot_ui_dists: ",LBL)
    # iterate layers
    for i, (ref, gen) in enumerate(zip(ref_us.T, gen_us.T)):
        ...
        gen_vals, _, _ = ax[0].hist(
            ...
            alpha=0.8, linewidth=1.0, color=color, label=LBL
        )
        # plot error bars
        ...
        handle2 = matplotlib.lines.Line2D([], [], c='#0000cc', label=LBL)
