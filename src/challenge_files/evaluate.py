...
def _to_numpy(x):
    if isinstance(x, torch.Tensor):
        # detach to break graph, move to CPU if needed, then convert
        return x.detach().cpu().numpy().copy()
    # ensures we actually make a copy even if already ndarray / list
...
def prepare_low_data_for_classifier(voxel_orig, E_inc_orig, hlf_class, label, cut=0.0, normed=False, single_energy=None):
    ...
    voxel = _to_numpy(voxel_orig)
    E_inc = _to_numpy(E_inc_orig)
    if normed:
        E_norm_rep = []
        E_norm = []
...
def train_cls(model, data_train, optim, epoch, arg):
    ...
    for i, data_batch in enumerate(data_train):
        ...
        # Ensure tensors are float and require gradients
        input_vector = input_vector.float().requires_grad_(True)
        target_vector = target_vector.float()
        
        # Debug prints (remove after fixing)
        # print(f"Input dtype: {input_vector.dtype}, requires_grad: {input_vector.requires_grad}")
        # print(f"Target dtype: {target_vector.dtype}, requires_grad: {target_vector.requires_grad}")
        output_vector = model(input_vector)
        criterion = torch.nn.BCEWithLogitsLoss()
         # Ensure output requires gradients
        # if not output_vector.requires_grad:
        #     print("WARNING: Model output doesn't require gradients!")
...
def run_from_py(sample, energy, doc, params,name=''):
    ...
    args = args_class(params)
    args.output_dir = doc.basedir + "/eval/" + name + "/"
    ...
    hlf = HLF.HighLevelFeatures(particle,
                                filename=params.get('xml_filename'))
    ...
    if args.mode in ['all', 'all-cls', 'cls-low', 'cls-high', 'cls-low-normed']:
        ...
        for key in list_cls:
            ...
            combined_path = os.path.join(
                args.output_dir,
                f"classifier_features_{args.mode}_{key}_{args.dataset}.npz"
            )
            ...
            print(f"Saved classifier features to {combined_path}")
