...
#from XMLHandler import XMLHandler #changed by Farzana
class HighLevelFeatures:
    ...
    def _calculate_EC(self, eta, phi, energy):
        if isinstance(energy, torch.Tensor):
            energy = energy.cpu().numpy()  # convert to numpy if it's a torch tensor
    ...
    def _calculate_sparsity(self, layer_data):
        ...
        # print("Shape of layer_data: ",layer_data.shape)
        # #.float() is added by me
        # return (layer_data > 0).float().mean(axis=1)
        
        #print("Shape of layer_data: ", layer_data.shape)
        if isinstance(layer_data, torch.Tensor):
            return (layer_data > 0).float().mean(dim=1)
        else:  # numpy array
            return (layer_data > 0).astype(np.float32).mean(axis=1)
