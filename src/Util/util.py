...
def load_data(filename, particle_type,  xml_filename, threshold=1e-5, single_energy=None):
    ...
    #print("layer_boundaries: ",layer_boundaries,single_energy)
    ...
    print("in utils: ",filename,xml_filename)
    ...
    # print("shape of energy_mask: ",energy_mask.shape)
    # print("shape of showers: ",data_file['showers'].shape)
...
def get_energy_and_sorted_layers(data):
    ...
    for layer_index in range(number_of_layers):
        ...
        #print("shape of layer in the loop: ",data[layer].shape )
        layers.append(data[layer])
       
    layers = np.concatenate(layers, axis=1)
    print("shape of layers: ",layers.shape, energy.min(), energy.max())
