...
class CaloChallengeDataset(Dataset):
    ...
    def __init__(self, hdf5_file, particle_type, xml_filename, val_frac=0.3, 
        ...
        del self.voxels
        print("after get energy call", self.energy.shape, self.energy.min(), self.layers.min(), self.energy.max())       
        ...
        print("Before calling the transform function : ", self.energy.shape,self.layers.shape, self.energy.min(), self.layers.min(), self.energy.max())
